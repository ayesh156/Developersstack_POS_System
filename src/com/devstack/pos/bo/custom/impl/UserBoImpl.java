...
public class UserBoImpl implements UserBo {
    ...
    public void initializeSystem() {
        if(!userRoleDao.isExists()){
            try(Session session = HibernateUtil.getSession()) {
                ...
                systemUser.setUsername("ayeshchathuranga.info@gmail.com");
                systemUser.setPassword(PasswordGenerator.passwordGen(6));
                systemUser.setDisplayName("Ayesh Chathuranga");
    ...
    public List<UserDto> loadAllUsers(String searchText) {
        ...
        for (User user: userDao.loadAllUsers(searchText)
            ...
            dtos.add(
                    new UserDto(user.getPropertyId(),user.getUsername(),
                            user.getPassword(), user.getDisplayName(),user.isActiveState(),
                            new UserRoleDto(user.getUserRole().getPropertyId(), user.getUserRole().getRoleName(),
    ...
    @Override
    public void updateSystemUser(Long roleId, String displayName, String email, Long userId) {
        userDao.updateSystemUser(roleId, displayName, email, userId);
    }
